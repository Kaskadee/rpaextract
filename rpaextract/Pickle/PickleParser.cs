...
namespace rpaextract.Pickle {
    ...
    internal sealed class PickleParser {
        ...
        /// <summary>
        ///     The protocol identifier. The next byte indicates the pickle version. All pickles created with protocol 2.0 or
        ///     greater have this at their beginning.
        /// </summary>
        private const char ProtocolIdentifier = '\x80';

        /// <summary>
        ///     Indicates a following short string which is less than 256 bytes long. The first byte indicates the length of the
        ///     string.
        /// </summary>
        ...
        ///     The python documentation of pickle.py is a little bit misleading. The actual value type is depending on the next
        ///     byte indicating the length of the byte sequence (less than 256 bytes according to the documentation)
        ...
        ///     In this case, the archive index will be created by popping the three top items from the stack. (Archive Index
        ///     without Prefix)
        /// </summary>
        ...
        ///     In this case, the archive index will be created by popping the four top items from the stack. (Archive Index with
        ///     Prefix)
        /// </summary>
                ...
                while (ms.Position < ms.Length) {
                    ...
                    switch (b) {
                        ...
                        case EndIndex: {
                            ...
                            if (!stack.TryPop(out var lengthObj) || !stack.TryPop(out offsetObj) ||
                                !stack.TryPop(out var pathObj)) {
                                ...
                                Console.WriteLine(
                                    $"(Warning) Failed to pop sufficient values from stack. (at mem-pos: {ms.Position})");
                                continue;
                            }

                            indices.Add(new ArchiveIndex((string) pathObj, Convert.ToInt64(offsetObj),
                                Convert.ToInt32(lengthObj), null));
                        ...
                        case EndIndexPrefix: {
                            ...
                            if (!stack.TryPop(out var prefixObj) || !stack.TryPop(out lengthObj) ||
                                !stack.TryPop(out offsetObj) || !stack.TryPop(out var pathObj)) {
                                // Push popped values back
                                if (prefixObj != null)
                            ...
                            indices.Add(new ArchiveIndex(Encoding.UTF8.GetString((byte[]) pathObj),
                                Convert.ToInt64(offsetObj), Convert.ToInt32(lengthObj), (byte[]) prefixObj));
                            // Skip next three bytes
                            ms.Position += 3;
                        }
                            break;
